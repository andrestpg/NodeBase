...
const getData = () => {
    ...
    $.get(url).done( async (res) => {
        $.each(res, (i, dt) => {
            i += 1;
            const roleName = getRoleName(dt.role)
            data = [i, `<p class="font-weight-bold text-primary m-0">${dt.name}</p>`, dt.username, roleName,  generateOpr(dt.id, dt.name, dt.username, dt.role)];
            dataset.push(data);
        });
    }).fail(() => {
...
const generateOpr = (id, name, username, role) => {
    ...
    userRole != 0 && (
        opr = `<div class="btn-group" role="group">
                    <button data-id="${id}" class="btn btn-info shadow btn-xs sharp mr-1" data-toggle="modal" data-target="#editModal" data-id="${id}" data-name="${name}" data-username="${username}" data-role="${role}"><i class="fa fa-pencil"></i></button>
                    <button type="button" onclick="delConfirm('users','${id}')" data-id="${id}" class="btn btn-danger shadow btn-xs sharp" data-toggle="tooltip" data-placement="top" title="Delete"><i class="fa fa-trash"></i></button>
                </div>`
    );
    return opr;
}
...
const appendData = async (id, name, username, role) => {
    const roleName = await getRoleName(role);
    myTable.row.add([ myTable.rows().count()+1, `<p class="font-weight-bold text-primary m-0">${name}</p>`, username, roleName,  generateOpr(id, name, username)]).draw().node();
}
...
const getRoleName = (role) => {
    let roleText = `<span class="badge light badge-success">Staff</span>`;
    role != 0 && (roleText = `<span class="badge light badge-secondary">Admin</span>`);
    return roleText;
}
